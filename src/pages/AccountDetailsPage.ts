...
export class AccountDetailsPage {
    ...
    readonly downloadTxnError: Locator;
    ...
    constructor(page: Page) {
        ...
        this.downloadTxnError = page.locator("//span[contains(text(),'We are unable to process your request. Please try ')]");
   ...
   */
    ...
    async VerifyPostedTransactionDetailsOfEnrichAccounts(AccountType?: string) {
        try {
            await this.page.waitForLoadState('load');
            const normalizedAccountType = (AccountType ?? '').trim().toUpperCase();
            const preferredIndex = normalizedAccountType === 'NOW' ? 1 : 0;
            const pickByIndex = async (selector: string): Promise<Locator> => {
                const locator = this.page.locator(selector);
                return (await locator.count()) > preferredIndex ? locator.nth(preferredIndex) : locator.first();
            };

            const updateCatTag = await pickByIndex("label[name='updateCatTag']");
            const transactionCategoryLabel = await pickByIndex('xpath=//label[@name="transactionCategoryLabel"]');
            const categoryEditIcon = await pickByIndex('xpath=//button[@name="categoryEditIcon"]');
            const categoryAddIcon = await pickByIndex('xpath=//button[@name="categoryAddIcon"]');
            const tagTitleLabel = await pickByIndex('xpath=//label[@name="tagTitleLabel"]');
            const tagAddIcon = await pickByIndex('xpath=//button[@name="tagAddIcon"]');
            const tagEditIcon = await pickByIndex('xpath=//button[@name="tagEditIcon"]');
            ...
            if (noPostedMsgVisible) {
                // No transactions available scenario
                const msg = await this.noPostTran.textContent();
                await Report.info(this.page, `No posted transactions available: ${msg ?? ''}`);
            } else {
                // Transactions available - verify first transaction details
                const firstRecordVisible = await this.clickingOnFirstRecordInPostedTran.isVisible({ timeout: 10000 }).catch(() => false);
                ...
                if (firstRecordVisible) {
                    ...
                    await expect(updateCatTag).toHaveText('Update Category or Tag');
                    await expect(transactionCategoryLabel).toHaveText('Category:');
                    const categoryAddVisible = await categoryAddIcon.isVisible().catch(() => false);
                    const categoryEditVisible = await categoryEditIcon.isVisible().catch(() => false);
                    if (categoryAddVisible) {
                        await expect(categoryAddIcon).toBeVisible();
                    }
                    if (categoryEditVisible) {
                        await expect(categoryEditIcon).toBeVisible();
                    }
                    if (!categoryAddVisible && !categoryEditVisible) {
                        await Report.info(this.page, 'Category icon is not displayed for this transaction (optional scenario).');
                    }
                    await expect(tagTitleLabel).toHaveText('Tag:');


                    const tagAddVisible = await tagAddIcon.isVisible().catch(() => false);
                    const tagEditVisible = await tagEditIcon.isVisible().catch(() => false);
                    if (tagAddVisible) {
                        await expect(tagAddIcon).toBeVisible();
                    }
                    if (tagEditVisible) {
                        await expect(tagEditIcon).toBeVisible();
                    }
                    if (!tagAddVisible && !tagEditVisible) {
                        await Report.info(this.page, 'Tag icon is not displayed for this transaction (optional scenario).');
                    }

                    await Report.pass(this.page, 'Expected details are displayed for posted transaction');
                    await this.closeTransactionDetailsPopup.click();
                    await this.page.waitForLoadState('load');

                } else {
                    await Report.fail(this.page, 'No posted transaction records found to verify');
                }
            }
        }
        catch (error) {
 ...
 /** <summary>
   ...
   */
    ...
    async VerifyPostedTransactionDetailsForLOC() {
        try {
            if(await this.noPostTran.isVisible().catch(() => false)){
                const noPostMsg = await this.noPostTran.textContent();
                await Report.info(this.page, `No posted transactions available: ${noPostMsg ?? ''}`);
                return;
            }
            await this.clickingOnFirstRecordInPostedTran.click();
   ...
   /*Summary>
    ...
    async ValidatePrintPreviewContent(accountLast4Digits: string, accountType?: string) {
        try {
            ...
            if(accountLast4Digits.includes('-')) {
                accountLast4Digits = accountLast4Digits.split('-').slice(-1)[0]; // Extract last part after hyphen
            ...
            // IRA accounts do not render an Available Balance/Available Credit line in print preview.
            if (normalizedAccountType === 'IRA') {
                await Report.info(this.page, 'Account type is IRA - skipping Available Balance/Credit validation in print preview');
            } else {
                ...
                const escapedAvailableFundsLabel = availableFundsLabel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                const availableFunds = frame.locator('.textLayer span').filter({ hasText: new RegExp(`^${escapedAvailableFundsLabel}`) }).first();
            ...
            const getNormalizedPreviewText = async (): Promise<string> => {
                const allSpansText = await frame.locator('.textLayer span').allTextContents();
                return this.normalizePrintPreviewValue(allSpansText.join(' '));
            };

            const includesWord = (source: string, token: string): boolean => {
            ...
            const normalizedPdfText = await getNormalizedPreviewText();

            for (const header of headers) {
                const normalizedHeader = this.normalizePrintPreviewValue(header);
                if (!normalizedPdfText.includes(normalizedHeader)) {
            ...
            for (const section of sectionsToValidate) {
                const noDataVisible = normalizedPdfText.includes(this.normalizePrintPreviewValue(section.noDataMsg));
                if (noDataVisible) {
                    ...
                    for (const col of section.colHeaders) {
                        if (!includesWord(normalizedPdfText, this.normalizePrintPreviewValue(col))) {
            ...
            const normalizedPdfTextAfterScroll = await getNormalizedPreviewText();
            const postedAnchor = this.normalizePrintPreviewValue('Posted Transactions:');
            const postedAnchorIndex = normalizedPdfTextAfterScroll.lastIndexOf(postedAnchor);
            const postedSectionText = postedAnchorIndex >= 0
                ? normalizedPdfTextAfterScroll.slice(postedAnchorIndex)
                : normalizedPdfTextAfterScroll;
            const noPostedTransactionsMessage = this.normalizePrintPreviewValue('There are no posted transactions.');

            if (postedSectionText.includes(noPostedTransactionsMessage)) {
                await Report.pass(this.page, 'No posted transactions present - skipping Posted Transactions column header validation');
                return;
            }

            const isNonEnrichAccount =
                normalizedAccountType === 'NHS' ||
                normalizedAccountType === 'IRA' ||
                normalizedAccountType === 'LOC';
            ...
            for (const col of postedColumnHeaders) {
                if (!includesWord(postedSectionText, this.normalizePrintPreviewValue(col))) {
            ...
            if (isNonEnrichAccount) {
                ...
                for (const enrichHeader of enrichOnlyHeaders) {
                    if (includesWord(postedSectionText, this.normalizePrintPreviewValue(enrichHeader))) {
 ...
 /*Summary>
    ...
    async ValidatePrintPreviewFirstRowData(capturedData: { pending: { date: string; description: string; amount: string }; posted: { date: string; description: string; amount: string; balance: string; vendor?: string; category?: string; tag?: string } }, accountType?: string) {
        try {
            ...
            //When There are no pending transactions. The print preview does not render the Pending Transactions table at all, so we only validate pending row data if a pending date was captured (which indicates at least 1 pending transaction was present) 
            ...
            if (capturedData.posted.date) {
                const normalizedAccountType = (accountType ?? '').trim().toUpperCase();
                const isNonEnrichByType = normalizedAccountType === 'NHS' || normalizedAccountType === 'IRA' || normalizedAccountType === 'LOC';
    ...
    /*Summary>
       ...
       async ClickOnDownloadButton() {
        try {
            ...
            if (await this.downloadTransactionsLabel.isVisible()) {
    ...
    async clickExportButton() {
        try {
            ...
            // Check for error UI after spinner
            if (await this.downloadTxnError.isVisible().catch(() => false)) {
                const errorMessage = await this.downloadTxnError.textContent() ?? '';
                throw new Error(`Download Transactions error message displayed: "${errorMessage.trim()}"`);
            }

            // Wait for the success dialog before registering the download listener —
            // the actual file download is triggered by closing the dialog, not by clicking Export.
            ...
            await Report.fail(this.page, `Export Failed due to : ${(error as Error).message}`);
            throw error;
        }
    } 
    async selectTransactionPeriod() {
        try {
            ...
            await Report.fail(this.page, `Failed to perform export transaction/Select Transaction Period : ${error.message}`);
            throw error;
        }
    } 
    ...
    async selectCustomDateRange(startDate: string, endDate: string) {
        try {
            ...
            await Report.fail(this.page, `Failed to perform export transaction/Select Date Range: ${error.message}`);
            throw error;
        }
    }
    ...
    async ValidateDownloadTransactionsFunctionality() {
        try {
            ...
            await Report.fail(this.page, `Failed to validate downloaded transactions file: ${(error as Error).message}`);
            throw error;
        }
    }
    ...
    async VerifyQuickActionsInAccountDetailsPage(last4DigitOfAcctNum: string, quickActionType: string) {
        try {
            const expectedActions = testdata?.QuickActionsAcctDetailsPage?.[quickActionType] as string[];
            if (!Array.isArray(expectedActions) || expectedActions.length === 0) {
                throw new Error(`QuickActionsAcctDetailsPage.${quickActionType} is missing or empty in testData.<env>.json`);
            }

            const normalize = (value: string) => value.replace(/[®™]/g, '').trim().replace(/\s+/g, ' ').toLowerCase();

            // Locate quick action buttons in the account details page
            const actionLabels = this.page.locator(`xpath=//ul[@role="list"]/li//button//span[2]`);

            const hasActions = await actionLabels.first().isVisible({ timeout: 15000 }).catch(() => false);
            if (!hasActions) {
                throw new Error(`No quick actions found on account details page for account ****${last4DigitOfAcctNum}`);
            }

            // Filter out "More" button from the initial actions list - it's handled separately
            const rawUiActions = (await actionLabels.allInnerTexts())
                .map((text) => text.trim())
                .filter((text) => text.length > 0 && normalize(text) !== 'more');
            const uiActions = Array.from(new Set(rawUiActions));
            const normalizedToUiAction = new Map<string, string>();
            for (const action of uiActions) {
                const norm = normalize(action);
                if (!normalizedToUiAction.has(norm)) {
                    normalizedToUiAction.set(norm, action);
                }
            }

            const expectedNormSet = new Set(expectedActions.map((action) => normalize(action)));
            const uiNormSet = new Set(uiActions.map((action) => normalize(action)));

            const missingActions = expectedActions.filter((action) => !uiNormSet.has(normalize(action)));
            const extraActions = uiActions.filter((action) => !expectedNormSet.has(normalize(action)));

            if (missingActions.length > 0 || extraActions.length > 0) {
                throw new Error(
                    `Quick action mismatch on Account Details page for ${quickActionType} account ****${last4DigitOfAcctNum}. ` +
                    `Expected: [${expectedActions.join(', ')}], UI: [${uiActions.join(', ')}], ` +
                    `Missing: [${missingActions.join(', ')}], Extra: [${extraActions.join(', ')}]`
                );
            }

            await Report.pass(this.page, `${quickActionType} account details quick actions match testData: [${expectedActions.join(', ')}]`);

            for (const expectedAction of expectedActions) {
                await this.closeBlockingDialogs();

                const uiActionLabel = normalizedToUiAction.get(normalize(expectedAction));
                if (!uiActionLabel) {
                    throw new Error(`Unable to resolve UI label for quick action '${expectedAction}'`);
                }

                const strippedLabel = uiActionLabel.replace(/[®™]/g, '').trim();
                const actionButton = this.page.locator(
                    `xpath=//ul[@role="list"]/li//button[.//span[2][contains(normalize-space(text()),"${strippedLabel}")]]`
                ).first();

                await expect(actionButton, `Quick action '${expectedAction}' is not visible on account details page`).toBeVisible({ timeout: 15000 });

                const popupPromise = this.page.context().waitForEvent('page', { timeout: 3000 }).catch(() => null);
                ...
                if (popupPage) {
                    ...
                    const popupValid = /zelle|transfer|documents|card|payment|billpay/i.test(popupUrl);
                    expect(popupValid, `Quick action '${expectedAction}' opened unexpected popup URL: ${popupUrl}`).toBeTruthy();
                    await popupPage.close();
                } else {
                    await waitForSpinnerToClose(this.page);
                    await this.page.waitForLoadState('domcontentloaded');

                    const isNavigationValid = await this.waitForExpectedAcctDetailsQuickActionDestination(normalize(expectedAction));
                    expect(
                        isNavigationValid,
                        `Quick action '${expectedAction}' did not navigate to expected destination. Current URL: ${this.page.url()}`
                    ).toBeTruthy();
                }

                await Report.pass(this.page, `Quick action '${expectedAction}' is visible and functional on account details page`);

                // Navigate back to dashboard via Accounts tab, then click account again
                await this.navigateBackToAccountDetails(last4DigitOfAcctNum);
            }

            // Validate additional actions after clicking "More" button
            const expectedAdditionalActions = testdata?.AdditionalActionsAcctDetailsPage?.[quickActionType] as string[];
            if (Array.isArray(expectedAdditionalActions) && expectedAdditionalActions.length > 0) {
                await this.VerifyAdditionalActionsAfterMoreButton(last4DigitOfAcctNum, quickActionType, expectedAdditionalActions);
            }

            await Report.pass(this.page, `Validated all ${quickActionType} account details quick actions for account ****${last4DigitOfAcctNum}`);
        } catch (error) {
            await Report.fail(this.page, `VerifyQuickActionsInAccountDetailsPage failed: ${(error as Error).message}`);
            throw error;
        }
    }

    private async VerifyAdditionalActionsAfterMoreButton(last4DigitOfAcctNum: string, quickActionType: string, expectedAdditionalActions: string[]) {
        try {
            await this.closeBlockingDialogs();

            // Target the button element directly whose span[2] label is exactly "More"
            const moreButton = this.page.locator(`xpath=//ul[@role="list"]/li//button[.//span[2][normalize-space(text())='More']]`
            ).first();

            const isMoreButtonVisible = await moreButton.isVisible({ timeout: 5000 }).catch(() => false);
            if (!isMoreButtonVisible) {
                await Report.pass(this.page, `No "More" button found - all actions are visible for ${quickActionType}`);
                return;
            }

            await moreButton.click();
            // Wait for the expanded additional actions to become visible
            await this.page.waitForTimeout(1500);
            await waitForSpinnerToClose(this.page);

            const normalize = (value: string) => value.replace(/[®™]/g, '').trim().replace(/\s+/g, ' ').toLowerCase();

            // Additional actions may appear in:
            // (a) the same ul[@role="list"] as new li items, or
            // (b) a dropdown/panel in a different container
            // Collect from both; exclude the original quick action labels and "More"
            const originalActionNorms = new Set(
                (testdata?.QuickActionsAcctDetailsPage?.[quickActionType] as string[] ?? []).map(normalize)
            );

            const candidateLabels = this.page.locator(`xpath=//div[@content="Partial_More_Actions"]//li`);

            const rawCandidates = (await candidateLabels.allInnerTexts())
                .map((text) => text.trim())
                .filter((text) => text.length > 0);
            const allUiLabels = Array.from(new Set(rawCandidates));

            // Keep only labels that are NOT in the original quick actions and NOT "More"
            const uiAdditionalActions = allUiLabels.filter(
                (text) => normalize(text) !== 'more' && !originalActionNorms.has(normalize(text))
            );

            const normalizedToAdditionalAction = new Map<string, string>();
            for (const action of uiAdditionalActions) {
                const norm = normalize(action);
                if (!normalizedToAdditionalAction.has(norm)) {
                    normalizedToAdditionalAction.set(norm, action);
                }
            }

            const expectedNormSet = new Set(expectedAdditionalActions.map((action) => normalize(action)));
            const uiNormSet = new Set(uiAdditionalActions.map((action) => normalize(action)));

            const missingAdditionalActions = expectedAdditionalActions.filter((action) => !uiNormSet.has(normalize(action)));
            const extraAdditionalActions = uiAdditionalActions.filter((action) => !expectedNormSet.has(normalize(action)));

            if (missingAdditionalActions.length > 0 || extraAdditionalActions.length > 0) {
                throw new Error(
                    `Additional action mismatch after "More" button for ${quickActionType} account ****${last4DigitOfAcctNum}. ` +
                    `Expected: [${expectedAdditionalActions.join(', ')}], UI: [${uiAdditionalActions.join(', ')}], ` +
                    `Missing: [${missingAdditionalActions.join(', ')}], Extra: [${extraAdditionalActions.join(', ')}]`
                );
            }

            await Report.pass(this.page, `Additional actions after "More" button match testData for ${quickActionType}: [${expectedAdditionalActions.join(', ')}]`);

            const actionFailures: string[] = [];

            for (const expectedAdditionalAction of expectedAdditionalActions) {
                try {
                    await this.closeBlockingDialogs();

                    const uiActionLabel = normalizedToAdditionalAction.get(normalize(expectedAdditionalAction));
                    if (!uiActionLabel) {
                        throw new Error(`Unable to resolve UI label for additional action '${expectedAdditionalAction}'`);
                    }

                    const strippedLabel = uiActionLabel.replace(/[®™]/g, '').trim();
                    const additionalActionButton = this.page.locator(
                        `xpath=//div[@content="Partial_More_Actions"]//span[contains(normalize-space(text()),"${strippedLabel}")]`).nth(0);

                    await expect(additionalActionButton, `Additional action '${expectedAdditionalAction}' is not visible on account details page`).toBeVisible({ timeout: 15000 });

                    const popupPromise = this.page.context().waitForEvent('page', { timeout: 3000 }).catch(() => null);
                    await additionalActionButton.click();
                    await waitForSpinnerToClose(this.page);
                    ...
                    if (popupPage) {
                        await popupPage.waitForLoadState('domcontentloaded');
                        const popupUrl = popupPage.url();
                        const popupValid = /statement|document|delivery|card|overdraft|check|payment|scheduled/i.test(popupUrl);
                        if (!popupValid) {
                            await popupPage.close();
                            throw new Error(`Additional action '${expectedAdditionalAction}' opened unexpected popup URL: ${popupUrl}`);
                        }
                        await popupPage.close();
                    } else {
                        await waitForSpinnerToClose(this.page);
                        ...
                        const isNavigationValid = await this.waitForExpectedAdditionalActionDestination(normalize(expectedAdditionalAction));
                        if (!isNavigationValid) {
                            throw new Error(`Additional action '${expectedAdditionalAction}' did not navigate to expected destination. Current URL: ${this.page.url()}`);
                        }
                    }

                    await Report.pass(this.page, `Additional action '${expectedAdditionalAction}' is visible and functional`);
                } catch (actionError: any) {
                    const msg = (actionError as Error).message;
                    actionFailures.push(`[${expectedAdditionalAction}]: ${msg}`);
                    await Report.fail(this.page, `Additional action '${expectedAdditionalAction}' FAILED: ${msg}`);
                }

                // Always navigate back and re-open More so the next action can be verified
                try {
                    await this.navigateBackToAccountDetails(last4DigitOfAcctNum);

                    const moreBtn = this.page.locator(`xpath=//ul[@role="list"]/li//button[.//span[2][normalize-space(text())='More']]`).first();
                    const isMoreBtnStillVisible = await moreBtn.isVisible({ timeout: 5000 }).catch(() => false);
                    if (isMoreBtnStillVisible) {
                        await moreBtn.click();
                        await this.page.waitForTimeout(1000);
                    }
                } catch {
                    // Navigation recovery failed — stop iterating to avoid cascading errors
                    break;
                }
            }

            if (actionFailures.length > 0) {
                throw new Error(`${actionFailures.length} additional action(s) failed:\n${actionFailures.join('\n')}`);
            }

            await Report.pass(this.page, `Validated all additional actions after "More" button for ${quickActionType} account ****${last4DigitOfAcctNum}`);
        } catch (error) {
            await Report.fail(this.page, `VerifyAdditionalActionsAfterMoreButton failed: ${(error as Error).message}`);
            throw error;
        }
    }

    private async waitForExpectedAcctDetailsQuickActionDestination(actionName: string): Promise<boolean> {
        const destinationLocators: Record<string, { locator: Locator; urlPattern: RegExp }> = {
            'make a transfer': {
                locator: this.page.getByLabel('Transfers', { exact: true }),
                urlPattern: /Transfers/i,
            },
            'transfer': {
                locator: this.page.getByLabel('Transfers', { exact: true }),
                urlPattern: /Transfers/i,
            ...
            'zelle': {
                locator: this.page.locator(`a[title='Send or Receive Money'], a[title='Manage Payment accounts'], button[name='buttonZelleEnroll']`)
                    .first()
                    .or(this.page.getByRole('heading', { name: 'Zelle Disclosure' })),
                urlPattern: /Zelle/i,
            },
            'manage card': {
                locator: this.page.getByText('Card Controls').first(),
                urlPattern: /Card|MyCards/i,
            },
            'view documents': {
                locator: this.page.locator(`a[title='Statements'], a[title='Notices'], a[title='Tax Documents']`).first(),
                urlPattern: /Documents|Document\s*Center/i,
            },
            'make a payment': {
                locator: this.page.getByLabel('Transfers', { exact: true })
                    .or(this.page.locator(`a[title='Pay Your Bills']`))
                    .or(this.page.locator(`label:has-text('Credit Card Payments')`).first()),
                urlPattern: /Transfers|BillPay|CreditCard/i,
            },
        };

        const dest = destinationLocators[actionName];
        if (!dest) {
            throw new Error(`No navigation verification defined for account details quick action '${actionName}'`);
        }

        try {
            await expect(dest.locator).toBeVisible({ timeout: 15000 });
            return true;
        } catch {
            return dest.urlPattern.test(this.page.url());
        }
    }

    private async waitForExpectedAdditionalActionDestination(actionName: string): Promise<boolean> {
        const additionalDestinationLocators: Record<string, { locator: Locator; urlPattern: RegExp }> = {
            'manage cards': {
                locator: this.page.getByText('Card Controls').first(),
                urlPattern: /Card|MyCards/i,
            },
            'view documents': {
                locator: this.page.locator(`a[title='Statements'], a[title='Notices'], a[title='Tax Documents']`).first(),
                urlPattern: /Documents|Document\s*Center|Statement/i,
            },
            'document delivery preferences': {
                locator: this.page.locator(`//label[contains(text(),'Document Delivery')]`).first(),
                urlPattern: /Delivery|Document|Preference/i,
            },
            'manage overdraft privilege': {
                locator: this.page.locator(
                    `//li[contains(@class,'active') or contains(@class,'selected')][.//span[contains(text(),'Overdraft')]] | ` +
                    `//a[@aria-selected='true' and contains(normalize-space(.),'Overdraft')] | ` +
                    `//tab-heading[@class='active']//span[contains(text(),'Overdraft')]`
                ).first(),
                urlPattern: /overProtection/i,
            },
            'order checks': {
                locator: this.page.locator(
                    `//li[contains(@class,'active') or contains(@class,'selected')][.//span[contains(normalize-space(.),'Order Checks')]] | ` +
                    `//a[@aria-selected='true' and contains(normalize-space(.),'Order Checks')] | ` +
                    `//tab-heading[@class='active']//span[contains(normalize-space(.),'Order Checks')]`
                ).first(),
                urlPattern: /AccountServices|Check|Order/i,
            },
            'stop payment': {
                locator: this.page.locator(`//span[contains(text(),'Stop Payment')]`).first(),
                urlPattern: /stopMaintenance|Stop|Payment/i,
            },
            'view scheduled transactions': {
                locator: this.page.locator(`//span[contains(text(),'Scheduled')]`).first(),
                urlPattern: /Scheduled|Schedule/i,
            },
            'view statements': {
                locator: this.page.locator(`a[title='Statements']`).first(),
                urlPattern: /Statement|Document/i,
            },
        };

        const dest = additionalDestinationLocators[actionName];
        if (!dest) {
            // For unmapped actions, just verify the URL changed or page loaded
            return true;
        }

        const requireActiveTabValidation = new Set([
            'manage overdraft privilege',
            'order checks',
        ]);
        ...
        try {
            ...
            if (requireActiveTabValidation.has(actionName)) {
                return false;
            }
            return dest.urlPattern.test(this.page.url());
        }
    ...
    private async closeBlockingDialogs(): Promise<void> {
        for (let attempt = 0; attempt < 4; attempt++) {
            const modal = this.page.locator('modal-container[role="dialog"], .modal.fade.in').first();
            const hasModal = await modal.isVisible({ timeout: 1000 }).catch(() => false);
            if (!hasModal) break;

            const closeButton = this.page.locator(
                'modal-container[role="dialog"] button:is(:has-text("OK"), :has-text("Close"), :has-text("Cancel"), .app-dialog-close.close, [title="Close"]), ' +
                '.modal.fade.in button:is(:has-text("OK"), :has-text("Close"), :has-text("Cancel"), .app-dialog-close.close, [title="Close"])'
            ).first();
    ...
    private async navigateBackToAccountDetails(last4DigitOfAcctNum: string): Promise<void> {
        await this.closeBlockingDialogs();

        // Always navigate directly to #/Main to fully reset the SPA routing state.
        // Relying on the "Accounts" tab click alone leaves a stale AccountServices context
        // in the SPA router, which causes subsequent quick-action clicks (e.g. Order Checks,
        // Manage Overdraft Privilege) to land on #/AccountServices instead of their real target.
        const currentUrl = new URL(this.page.url());
        await this.page.goto(`${currentUrl.origin}${currentUrl.pathname}#/Main`, { waitUntil: 'domcontentloaded' });
        await waitForSpinnerToClose(this.page);

        await this.closeBlockingDialogs();

        // Click the specific account to navigate back to account details
        const accountLink = this.page.locator(`xpath=//*[text()="****${last4DigitOfAcctNum}"]`);
        await expect(accountLink, `Account ****${last4DigitOfAcctNum} not found on dashboard`).toBeVisible({ timeout: 15000 });
        await accountLink.click();
        await this.page.waitForTimeout(2000);
        await this.page.waitForLoadState('load');
        await waitForSpinnerToClose(this.page);
        await expect(this.showAcctDetails).toBeVisible({ timeout: 15000 });
    }
